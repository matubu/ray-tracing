...
typedef struct s_camera {
	...
	float size_of_pixel;// angle of a pixel
}	t_camera;
...
t_color	mix(t_color *a, t_color *b, float fac)
{
	t_color color;
	if (fac > 1.0)
		fac = 1.0;
...
{
	t_pos	vec;
	float	yaw_sin = sinf(rot->z),
			yaw_cos = cosf(rot->z),
			pitch_sin = sinf(rot->y),
			pitch_cos = cosf(rot->y),
			roll_sin = sinf(rot->x),
			roll_cos = cosf(rot->x);
...
{
	printf("|/_ %f %f %f; -> %f %f %f\n", orig->x, orig->y, orig->z, ray->x, ray->y, ray->z);
...
void	render(t_scene *scene, t_color *data)
{
	...
	t_pos	vec;
	//t_pos	pos = scene->camera->pos;
	t_pos	rot = scene->camera->rot;

	float	angle_z = scene->camera->rot.z - (scene->camera->fv / 2.0 - .5 * scene->camera->size_of_pixel);
	//pos.z += (scene->camera->height / 2.0 - .5) * scene->camera->size_of_pixel;
	rot.x += (scene->camera->height / 2.0 - .5) * scene->camera->size_of_pixel;
	...
	for (unsigned int y = 0; y < scene->camera->height; y++)
	{
		...
		{
			vec = etov(&rot);
			ray_scene(&scene->camera->pos, &vec, scene, data + j++);
			rot.z += scene->camera->size_of_pixel;
			//pos.x += scene->camera->size_of_pixel;
		}
		rot.x += scene->camera->size_of_pixel;
		//pos.z -= scene->camera->size_of_pixel;
	}
}
