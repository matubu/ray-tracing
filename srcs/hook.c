...
/*   Updated: 2021/12/29 20:31:23 by mberger-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	hook_mouse_move(int x, int y, t_scene *scene)
{
	...
	if (!first)
	{
		...
		scene->cam.rot.y += ((float)(y - last.y) / 50.0) * cos(scene->cam.rot.z);
		scene->cam.rot.x += ((float)(y - last.y) / 50.0) * sin(scene->cam.rot.z);
